...
// Cambiar a true cuando la verificación de Google OAuth esté aprobada
...
export default function AuthModal({ mode, onModeChange, onClose }: AuthModalProps) {
  ...
  async function usernameExists(cleanUsername: string) {
    const { data, error } = await supabase
      .from("profiles").select("username").eq("username", cleanUsername).maybeSingle();
    if (error) { setAuthMessage(error.message); return true; }
    return !!data;
  }
  ...
  async function handleSubmit() {
    ...
    if (isSignup && (!cleanName || !cleanUsername)) { setAuthMessage("Introduce nombre y usuario."); return; }
    if (!email.trim() || !password.trim()) { setAuthMessage("Introduce email y contraseña."); return; }
    if (isSignup && cleanUsername.length < 3) { setAuthMessage("El usuario debe tener al menos 3 caracteres."); return; }

    setIsLoading(true);

    if (isSignup) {
      const exists = await usernameExists(cleanUsername);
      if (exists) { setIsLoading(false); setAuthMessage("Ese usuario ya está en uso."); return; }
    }

    const result = isSignup
      ? await supabase.auth.signUp({ email, password, options: { data: { name: cleanName, username: cleanUsername } } })
      : await supabase.auth.signInWithPassword({ email, password });

    setIsLoading(false);

    if (result.error) { setAuthMessage(result.error.message); return; }
    if (isSignup) { setAuthMessage("Cuenta creada. Revisa tu email para confirmar el registro."); return; }
